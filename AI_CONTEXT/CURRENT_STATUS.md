...
最後更新：2026-09-14（Claude Web Cloud session）
...
| 品牌改名「CK Japan Select」 | Deployed | Yes | 前後台所有可見品牌文字統一改名，2026-09-14 驗證上線 |
| 主題標籤改版（點擊塞入搜尋框，OR 比對合作商品） | Deployed | Yes | 不再是獨立篩選列，chip 移到搜尋框下方，2026-09-14 驗證上線 |
| 商品照片 Ctrl+V 直接貼上 + 貼上後自動收合編輯列 | Deployed | Yes | 後台 `shopping-admin.html`，2026-09-14 驗證上線 |
| Amazon 書籤工具縮圖列修復（`#altImages`） | Deployed | 無法用 probe.js 驗證（Amazon 擋伺服器請求） | 依 Amazon 多年沒變的標準結構補上，賣家需重新拖曳書籤才會生效 |
| intimissimi.com 通用解析誤收推薦商品圖修復 | Deployed | Yes | 排除 class 含 tile 的 itemprop=image 元素，20 項 extract.test.js 全過 |
